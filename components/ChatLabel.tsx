...
const ChatLabel = ({ openMenu, setOpenMenu, id, name, selected }) => {
  ...
  return (
    <div
      onClick={selectChat}
      className={`flex items-center justify-between p-2 rounded-lg text-sm group cursor-pointer transition-all
        ${selected ? 'bg-blue-500/20 border-l-4 border-blue-400 pl-2 text-white' : 'hover:bg-white/10 text-white/80'}
      `}
    >
      <p className="truncate max-w-[80%] group-hover:max-w-[80%]">{name}</p>
      <div
        onClick={e => {
          e.stopPropagation()
